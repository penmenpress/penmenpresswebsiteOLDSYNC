...
export class SubmitToWorkflow extends Component {
   ...
   /**
	...
	getFirstStep( workflowId ) {
      ...
      wp.apiFetch({ path: '/oasis-workflow/v1/workflows/submit/firstStep/workflowId=' + workflowId + '/postId=' + postId, method: 'GET' }).then(
         (stepdata) => {
   ...
   render() {
      const { isSaving, isCurrentPostPublished, isCurrentPostScheduled, postStatus, isPostInWorkflow, postMeta } = this.props;
      const { validationErrors, hideForm, workflowButtonText, assignActorLabel, publishDateLabel, dueDateLabel, displayPublishDate, displayDueDate, redirectingLoader,
      ...
      if ( ( postMeta && postMeta._oasis_is_in_workflow == "1" )  // post is in another workflow
            || isPostInWorkflow
            || hideForm
...
export default compose([
	withSelect((select) => {
      const { getCurrentPostId, 
               ...
               getCurrentPost } = select('core/editor');
