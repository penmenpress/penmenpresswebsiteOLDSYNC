...
function pp_has_group_cap( $cap_name, $group_id, $group_type ) {
	...
	if ( $group_id && ! $has_sitewide ) {
		...
		if ( in_array( $group_id, _pp_retrieve_admin_groups( $operation ) ) )
			$has_sitewide = true;
	}
...
function _pp_retrieve_admin_groups( $operation = 'manage' ) {
	return apply_filters( 'pp_admin_groups', array(), $operation );
}
