		...
		foreach ( array_keys($agent_types) as $agent_type ) {
			...
			if ( 'wp_role' == $agent_type ) {
				...
				foreach( $this->data->agent_info['wp_role'] as $agent_id => $role ) {
					if ( in_array( $role->metagroup_id, array( 'wp_anon', 'wp_all' ) ) && ( ! defined( 'PPFF_VERSION' ) || 'attachment' != $for_item_type ) && ! defined( 'PP_ALL_ANON_FULL_EXCEPTIONS' ) && ( ( 'read' != $op ) || pp_get_option( 'anonymous_unfiltered' ) ) )
						continue;
