...
function _ppc_item_ui_hints( $for_item_type ) {
	if ( ( 'attachment' == $for_item_type ) && ! defined( 'PPFF_VERSION' ) ) {
		...
		elseif( true == pp_key_status() )
			...
			$msg = sprintf( __( 'To block direct access to unreadable files, %1$spurchase a support key%2$s and install the PP File URL Filter plugin.', 'pp' ), '<a href="https://presspermit.com/purchase">', '</a>' );
	...
	if ( ! defined( 'PPCE_VERSION' ) ) {	
		...
		elseif( true == pp_key_status() )
			...
			$msg = sprintf( __( 'To customize editing permissions, %1$spurchase a support key%2$s and install the PP Collaborative Editing plugin.', 'pp' ), '<a href="https://presspermit.com/purchase">', '</a>' );
