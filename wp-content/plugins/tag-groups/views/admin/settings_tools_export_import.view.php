<div class="tg_settings_tabs_content">
  <p>
    <form method="POST" action="<?php esc_url( $_SERVER['REQUEST_URI'] ) ?>">
      <h3><?php _e( 'Export', 'tag-groups' ) ?><span class="dashicons dashicons-editor-help chatty-mango-help-icon" data-topic="export"></span></h3>
      <div class="chatty-mango-help-container chatty-mango-help-container-export" style="display:none;">
        ...
        <p><?php _e( 'Use this button to export all Tag Groups settings and groups and all terms that are assigned to a group into files.', 'tag-groups' ) ?></p>
        <p><?php _e( "You can import both files separately. Category hierarchy won't be saved. When you restore terms that were deleted, they receive new IDs and you must assign them to posts again. Exporting cannot substitute a backup.", 'tag-groups' ) ?></p>
      </div>
  ...
  <p>
    <form method="POST" action="<?php echo esc_url( $_SERVER['REQUEST_URI'] ) ?>" enctype="multipart/form-data">
      <h3><?php _e( 'Import', 'tag-groups' ) ?><span class="dashicons dashicons-editor-help chatty-mango-help-icon" data-topic="import"></span></h3>
      <div class="chatty-mango-help-container chatty-mango-help-container-import" style="display:none;">
        ...
        <p><?php _e( 'Below you can import previously exported settings/groups or terms from a file.', 'tag-groups' ) ?></p>
        <p><?php _e( 'It is recommended to back up the database of your site before proceeding.', 'tag-groups' ) ?> <?php _e( 'Please import the settings before the terms.', 'tag-groups' ) ?></p>
      </div>
